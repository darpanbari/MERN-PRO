...
const AdminOrders = () => {
  ...
  const handleChange = async (orderId, value) => {
    try {
      const { data } = await axios.put(`http://localhost:2020/product/order-status/${orderId}`, {
        status: value,
      });
      getOrders();
    } catch (error) {
  ...
  return (
    <>
      <div className="row p-3 dashboard">
        ...
        <div className="col-lg-10 col-md-6 shadow-lg my-5 dash margin" style={{width:"60%", marginLeft:"28%"}}>
          <h1 className="text-center my-5 fst-italic text-white bgcolor-2 py-4 ">All Orders</h1>
          {orders?.map((o, i) => {
            return (
              <div className="border shadow m-4">
